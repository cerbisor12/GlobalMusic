...
/**
 * Child Class of JPanel, generates a panel including smaller panels, one for each event
 *
 */
public class EventListPanel extends JPanel{
    /**
     *Create the panel including all events or only future events
     * @param showFutureEvents boolean
     */
    public EventListPanel(boolean showFutureEvents){
        ...
        query +="AND E.DateOfEvent>=NOW() ORDER BY E.DateOfEvent";}
            ...
            query += "ORDER BY E.DateOfEvent LIMIT 3";
        }
    ...
    /**
     * Create the panel according to a search criteria string
     * @param searchCriteria String - input search criteria
     */
    public EventListPanel(String searchCriteria) {
    	this.setBackground(Color.black);
    	this.setLayout(new BoxLayout(this, BoxLayout.Y_AXIS));
     ...
     * @param datePicker DatePicker -selected date by the user
     */
    public EventListPanel(DatePicker datePicker) {
    	this.setBackground(Color.black);
    	this.setLayout(new BoxLayout(this, BoxLayout.Y_AXIS));
     ...
     * @param query the mysql query from the constructor
    ...
    private List<List<String>> getResults(String query){
        ...
        try {
            ...
            while(rs.next()){
                ...
                boolean has = false;
                //check if event is already added on the array
                for (List<String> list : results){
                    if (list.get(1).equals(name)){
                        has =true;
                    }
                }
                //if the event is added as an array already, add only the artist
                if (has){
                    int index = results.indexOf(results.stream().filter(e -> e.contains(name)).findAny().get());
                        results.get(index).add(artist);
                }
                //else add all the event details
                else{
    ...
    /**
     * This method creates a panel for each event and adds it the parent panel
    ...
    private void createPanels(String query){
        ...
        for (int i=0;i<size;i++){
            ...
            //Load a newBookingView through NewBookingController
